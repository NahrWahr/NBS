...
function GetInitialCoOrdinates(Simulation)
...
function GravAcc!(dv,
    ...
    @inbounds for j = 1:n
        if j != 1
            ...
            Rij = Ri - Rj
            acc -= G * bodies[j].m * Rij / norm(Rij)^3
        end
    end
...
function DiffEqBase.ODEProblem(Simulation)
	(u0,v0,n) = GetInitialCoOrdinates(Simulation)
	AccFn = [GravAcc!]

	ODEsys! = let AccFn = tuple(AccFn...)
...
begin
	...
	body2 = MassBody(SVector(0.0, -1.0, 0.0), SVector(-1e-6, 0.0, 0.0), 20.0)
	G = 6.673e-11
	tspan = (0.0, 100000.0)
	gsystem = GravitationalSystem([body1,body2], G)
end

# ╔═╡ ab1b3dc2-810f-48bd-9ff0-782b491be7d3
unoproblemo = ODEProblem(Simulation(gsystem,tspan))

# ╔═╡ d45d9143-8789-4cc1-8237-f216f98a2538
solve(unoproblemo)
